...
function update() {
  ...
  // Move Bittu's paddle
  computerPaddleY += (ballY - (computerPaddleY + computerPaddle.offsetHeight / 2)) * 0.1; // Adjusted speed
  computerPaddleY = Math.max(0, Math.min(gameArea.offsetHeight - computerPaddle.offsetHeight, computerPaddleY)); // Keep within bounds
  computerPaddle.style.top = `${computerPaddleY}px`;

  // Check scoring
  if (ballX <= 0) {
    computerScore++;
    computerScoreDisplay.textContent = `Bittu: ${computerScore}`;
    resetBall();
  }
  ...
  if (playerScore >= 5 || computerScore >= 5) {
    endGame(playerScore >= 11 ? playerName : "Bittu");
    return;
  }
